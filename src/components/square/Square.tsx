const Square=({index, object, isSelected, setSelectedIndex, isPosibleNextMove}:{index: number, object: String, isSelected: boolean, setSelectedIndex:(e:number)=>void, isPosibleNextMove: boolean})=>{
    ...
    return(
        <div onClick={()=>setSelectedIndex(isSelected ? 0: index)} className={`w-full cursor-pointer aspect-square flex items-center justify-center relative ${isSelected ? "opacity-80" : ""}`} style={{backgroundColor:`${isDark ? "green": "whute"}`}}>
